/**
 * `withGulitars` is a shipped export with a job that only shows up on a device:
...
const { withGulitars, SINGLETONS } = require("../metro/index.js");
...
const configure = (options?: Record<string, unknown>) =>
	withGulitars({ projectRoot: APP, resolver: {} }, options);

describe("withGulitars", () => {
	it("is requireable from a CommonJS metro.config.js", () => {
		assert.equal(typeof withGulitars, "function");
